...
- **Action 修复**：修复 GitHub发布流程，确保 `languages/` 目录被正确包含在发布的 ZIP 包中，解决翻译文件丢失问题。

## v1.2.2 (2026-02-02)

### 🐛 修复与优化

- **日志优化**：重构日志记录机制，自定义 `cnblogs_sync_log` 函数，仅在 `WP_DEBUG` 开启时生效，避免生产环境产生大量日志。
- **本地化完善**：全面检查并补全中文翻译，包括 JS 动态文本和后台设置选项。
- **安全性增强**：增加日志数据的类型检查，增强插件稳定性。
